...
class YouTubeChannel {
  ...
  uploadVideo(videoTitle: string): void {
    ...
    for (const subcriber of this.subscribers) {
      subcriber.notify(videoTitle);
    }
  }
}
...
function main() {
  const channel = new YouTubeChannel("Cocinando con Brithany");

  const melissa = new Subcriber("Melissa");
  const cesar = new Subcriber("César");
  const emin = new Subcriber("Emin");
  ...
  channel.uploadVideo('Receta de pastel de chocolate');

  channel.unsubscribe(emin);

  channel.uploadVideo('Receta de parrillada');
  
  channel.unsubscribe(melissa);

  channel.uploadVideo('Receta de salmón a la plancha');
